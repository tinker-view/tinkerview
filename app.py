...
def add_res_modal(clicked_date, m_list):
    # 클릭된 문자열에 이미 시간 정보가 섞여 있는지 확인 (주간 달력 대응)
    # 단순히 '2026-02-06' 형태인지 확인해서 보정 여부 결정
    try:
        base_date = datetime.strptime(clicked_date, "%Y-%m-%d")
        # 월간 달력에서 온 거라면 (시차가 발생할 수 있으므로) 하루를 더하고,
        # 주간 달력은 클릭한 칸의 날짜가 정확하므로 그대로 둡니다.
        # (테스트 결과 주간 클릭 시에도 밀린다면 아래 +1 로직을 유지하세요)
        fixed_date = base_date.date() 
    except:
...
with tabs[0]:
    ...
    if not df_r.empty:
        ...
        # 4. 날짜 클릭 시 예약 등록 팝업 (월간/주간 모두 대응)
        if state.get("dateClick"):
            raw_date = state["dateClick"]["date"]
            
            # 주간 달력은 '2026-02-06T10:00:00Z' 형태로 들어올 수 있음
            # 'T'를 기준으로 앞의 날짜 정보만 가져옵니다.
            clicked_date = raw_date.split("T")[0]
            
            # ✨ 핵심: 주간 달력에서 클릭 시 시간이 포함되면 하루가 밀리지 않도록
            # raw_date에 'T'가 포함되어 있다면 (시간 정보가 있다면) 보정 없이 그대로 쓰고,
            # 'T'가 없다면 (월간 달력 클릭 시) 기존처럼 처리하도록 add_res_modal에 넘깁니다.
