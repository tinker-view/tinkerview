...
def load_data(sheet_name):
    # 💡 'stocks' 시트가 목록에 있어야 데이터가 로드됩니다! ㅋ
    expected = {
...
def get_stock_val(item_name):
    ...
    try:
        ...
        if not row.empty: return int(float(row['현재고'].values[0]))
    except: return 0
    return 0
...
# 탭 1: 스케줄 달력 뷰 (대장님 원래 설정 완벽 복원! ㅋ)
with tabs[0]:
    ...
    if state.get("callback") == "dateClick":
        raw_d = str(state["dateClick"]["date"])
        if "T" in raw_d and raw_d.split("T")[1][:8] != "00:00:00":
            if st.session_state.clicked_res_info != raw_d:
                st.session_state.clicked_res_info = raw_d; st.session_state.show_res_modal = True; st.rerun()
...
# 탭 5: 재고 관리 (핵심 기능 추가 완료! ㅋ)
with tabs[4]:
    st.subheader("📦 필수 재고 관리")
    if df_stock is None or df_stock.empty:
        st.error("🚨 'stocks' 시트를 읽어오지 못했습니다! 구글 시트에 'stocks' 시트가 있는지 확인해주세요.")
    else:
        col1, col2 = st.columns(2)
            ...
            with [col1, col2][i%2]:
                ...
                if st.button(f"{item} 반영", key=f"btn_{item}"):
                    if manage_gsheet("stocks", action="update_stock", key=item, extra={"new_total": str(cur + adj)}):
                        st.success("반영 성공!"); st.cache_data.clear(); st.rerun()
        st.divider()
